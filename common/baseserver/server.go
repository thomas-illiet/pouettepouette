...
//   - Started when base server is configured WithGRPCPort (port is non-negative)
...
//   - Started when base server is configured WithHTTPPort (port is non-negative)
...
type Server struct {
	...
	// listening indicates the server is serving. When closed,
	// the server is in the process of graceful termination.
